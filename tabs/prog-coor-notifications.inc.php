...
// $query = mysqli_query($conn, "SELECT * FROM notifications WHERE (type = 'shared'  and `user_ID` = '{$_SESSION['user_ID']}') or  department = (SELECT course_users FROM users WHERE user_ID = '{$_SESSION['user_ID']}') ORDER BY notif_date DESC LIMIT 15");
...
$query = mysqli_query($conn, "SELECT * FROM notifications INNER JOIN users  ON notifications.user_ID = users.user_ID  WHERE (type = 'shared'  and `receiver_ID` = '{$_SESSION['user_ID']}') or department = (SELECT course_users FROM users WHERE user_ID = '{$_SESSION['user_ID']}') and sem_ID = '$semIdquery' and notifications.user_ID <> '{$_SESSION['user_ID']}' ORDER BY notif_date DESC LIMIT 15");
$count1 = mysqli_num_rows($query);

$query2 = mysqli_query($conn, "SELECT * FROM notifications INNER JOIN users  ON notifications.user_ID = users.user_ID  WHERE (type = 'shared'  and `receiver_ID` = '{$_SESSION['user_ID']}') or department = (SELECT course_users FROM users WHERE user_ID = '{$_SESSION['user_ID']}') and `status`= 0 and sem_ID = '$semIdquery' and  notifications.user_ID <> '{$_SESSION['user_ID']}' ORDER BY notif_date DESC LIMIT 15");
...
<li class="nav-item dropdown">
    ...
    <div class="dropdown-menu dropdown-menu-end p-2" style="background:#f4e8ea;max-height: 700px;overflow-y:scroll;" aria-labelledby="navbarDropdown">
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            if ($row['status'] == 0) {
        ?>
                <a class="dropdown-item notif-item m-0" style="border-radius:10px;background:#b6d3f2;color:black" href="#" data-id="<?php echo $row['notif_ID'] ?>" data-type="<?php echo $row['message'] ?>">
        ...
        $('.notif-item').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $(".notif").load("../tabs/prog-coor-notifications.inc.php");
                    if (data.indexOf('document') >= 0 && data.indexOf('uploaded') >= 0) {
                        $("#doc_tab").trigger('click');
                    } else if (data.indexOf('report') >= 0 && data.indexOf('uploaded') >= 0) {
